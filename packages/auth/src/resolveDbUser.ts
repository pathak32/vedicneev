...
/**
 * Maps a Supabase-authenticated identity to this project's Postgres User
 * row. Three cases, in order:
 *
 ...
 *    cuid id as the app-level identity rather than creating a duplicate row
 *    under the new Supabase id — every other table's FK (Subscription,
 *    TestSession, MistakeVault, OfflineMockSession, InstituteAdmin) already
 *    points at that cuid, and phone is @unique so inserting a second row
 *    with the same phone under a different id would throw a P2002
 ...
 *
 * Shared by both apps/web (student/parent sign-in) and apps/omrtest
 * (institute admin sign-in) — an InstituteAdmin is a normal User under the
 * hood (see packages/db/prisma/schema.prisma's InstituteAdmin comment), so
 * this same identity resolution applies to both without a fork.
 */
