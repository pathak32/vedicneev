...
import { generateOtpCode, hashOtpCode, otpCodeMatches } from "./otpCrypto";
import { toSupabasePhoneDigits } from "./phoneFormat";
 ...
 * whatever WHATSAPP_ACCESS_TOKEN/WHATSAPP_PHONE_NUMBER_ID credentials the
 * calling app has configured for its own report/message sending.
 *
 ...
 * isn't configured, falls back to a plain phone-keyed User upsert, the same
 * demo-mode convention every other integration in this codebase uses, so
 * local/demo dev keeps working without live credentials.
 *
 * Extracted from apps/web (originally src/lib/auth/whatsappOtpServer.ts)
 * into this shared package so apps/omrtest's InstituteAdmin login can reuse
 * the exact same, already-tested flow instead of a second implementation —
 * an InstituteAdmin authenticates with the same phone-OTP-to-Supabase
 * bridge as any other User (see packages/db's InstituteAdmin comment);
 * only the authorization layered on top (is this User also an
 * InstituteAdmin) differs per app.
 *
 * Called by each app's own thin HTTP adapters — e.g.
 * apps/web/app/api/auth/whatsapp/send-otp+verify-otp/route.ts and
 * apps/omrtest's equivalent — so the send/verify halves stay two thin
 * adapters over one implementation instead of drifting apart.
 */
 ...
 * Supabase's magic-link Admin API requires — this project is phone-first
 * and has no real email for these users. Never written to Prisma's
 * User.email (that column is for a real address); purely an internal
 * Supabase Auth implementation detail, recomputed from phone on every
 * login rather than looked up, so no phone -> Supabase-user mapping needs
 * to be stored anywhere but our own User.id (see resolveDbUser.ts).
 */
 ...
 *    any real magic-link sign-in. createSupabaseServerClient's shared
 *    cookie domain (see env.ts) is what makes that resulting session
