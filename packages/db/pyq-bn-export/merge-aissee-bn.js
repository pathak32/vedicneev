...
const drafts = require("./aissee-bn-drafts.json");

const rows = withoutBn.filter((r) => r.examType === "AISSEE");
...
console.log("Total AISSEE rows needing bn:", rows.length);
...
fs.writeFileSync(path.join(__dirname, "aissee-merged-for-review.json"), JSON.stringify(merged, null, 2));
console.log("Wrote aissee-merged-for-review.json with", merged.length, "rows");
